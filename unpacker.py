...
def decompress_project(archive_path: Union[str, Path], extract_to_dir: Path, overwrite: bool = False) -> str:
    ...
    Args:
        ...
        overwrite (bool, optional): 如果为 True 且目标目录已存在，则会先删除该目录再进行解压。
                                    默认为 False，如果目标目录存在且非空，则会引发错误。
    ...
    Raises:
        ...
        FileExistsError: 如果目标目录已存在且不为空，并且 overwrite 参数为 False。
        ValueError: 如果文件格式不受支持或文件已损坏。
    """
