        ...
        [
            (
                ...
                (
                    ...
                    "当用户提供一个压缩包路径（例如：'请帮我总结一下 G:\\paper.zip'）并提出分析请求时，"
                    "你必须遵循以下步骤："
                    "1. 使用 `decompress_project` 工具将压缩包解压到项目下的 `./data` 目录。"
                    "   请务必使用 './data' 作为解压目标路径，并设置 `overwrite=True`。"
                    "2. 成功解压后，你会得到一个目录路径。接下来，必须使用 `extract_latex_text_from_folder` 工具，"
                    "   传入上一步得到的目录路径，以获取论文的文本内容。"
...
async def main():
    ...
    try:
        ...
        print("你好！我是你的AI阅读助手 (使用 OpenAI 模型)。我们可以聊任何话题，"
              "如果你需要分析PDF，随时告诉我它的路径。")
